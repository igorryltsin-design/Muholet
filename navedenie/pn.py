"""Законы наведения: метод пропорционального сближения (МПС), истинный МПС (TPN,
команда по нормали к линии визирования), МПС с компенсацией ускорения цели
(экспериментальный), метод погони, метод трёх
точек (CLOS), МПС по измерениям ГСН, экспериментальное МПС с переменным
...
# (см. docs/notation.md и docs/law_passports.md). «МПС» = метод пропорционального
...
def ppn_accel(r: Vec, v_m: Vec, v_t: Vec, n_const: float, n_max: float) -> Vec:
    """a_МПС = N · (ω_ЛВ × V_к) — размерностно корректный метод пропорционального сближения.
...
def tpn_accel(r: Vec, v_m: Vec, v_t: Vec, n_const: float, n_max: float) -> Vec:
    """Истинный МПС (TPN): a = N·V_c·(ω_ЛВ × r̂) — нормальное УСКОРЕНИЕ,
    ...
    Applications, 2021). Отличие от «обычного» МПС выше: множитель V_c вместо
...
def pn_seeker_accel(az_dot: float, el_dot: float, v_m: Vec, n_const: float, n_max: float) -> Vec:
    """«МПС по измерениям ГСН»: команда только из ИЗМЕРЕННЫХ углов и их скоростей.
...
def pure_pursuit_accel(r: Vec, v_m: Vec, n_max: float) -> Vec:
    ...
    случай МПС (5.15) при навигационном коэффициенте N=1 (у Гусева K≡N=1), C=0.
    ...
    годится лишь против скоростных целей и служит наглядным антиэталоном МПС."""
    desired = _unit(r)
    current = _unit(v_m)

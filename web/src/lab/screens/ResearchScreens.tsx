...
/** Экран «Карта»: карта преимуществ «муха против МПС» + коэволюция. */
export function MapScreen({
  ...
  return (
    <LabScreen
      title="Карта"
      about="Сетка «манёвр × закон наведения» (5 законов, включая истинный МПС): промах сохранённого мозга против эталона с тем же законом на встречных 8 км. Зелёная ячейка — муха точнее эталона, красная — хуже."
      primary={
          ...
          <>
                ...
                <tbody>
                  {['прямолинейно', 'вираж', 'змейка', 'ножницы', 'горка/пике'].map((m) => (
                    <tr key={m}>
                      ...
                      {[['pn', 'МПС'], ['tpn', 'истинный МПС'], ['apn', 'МПС+комп.'], ['pure', 'метод погони'], ['clos', 'метод трёх точек']].map(([id, l]) => {
            ...
            {mapData.rows[0]?.n_int_fly !== undefined && (
              <>
                ...
                <p className="lab-hint">Квадрат — муха, круг — МПС с тем же законом. Точка левее-ниже = точнее и экономичнее.</p>
              </>
            )}
            <p className="lab-hint">
              В ячейке — промах мухи и для сравнения промах эталона с этим законом. <b>Зелёная</b> — муха точнее (больше чем на 5 м),{' '}
              <b>красная</b> — хуже, серая — паритет. Истинный МПС (TPN) берёт команду по нормали к линии визирования и
              пропорционально закрывающей скорости — на боковых геометриях расходится с обычным МПС; на лобовых ячейки почти совпадают.
        ...
        {coev && (
          <>
            ...
            <p className="lab-hint">
              ...
              (фитнес самой опасной цели), пунктир — медиана популяции (насколько опасны «обычные» цели); красное кольцо — поколение,

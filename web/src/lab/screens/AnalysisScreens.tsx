...
export function CompareScreen({
  ...
  return (
    <LabScreen
      title="Сравнение"
      about="Текущий сценарий прогоняется на эталонном ПН, законах наведения и всех мозгах. Эталонный ПН видит точную геометрию, поэтому это сравнение «с подсказкой»."
      primary={
      ...
      note={
        <>
          Для сенсорных участников (био, «ПН через ГСН») отклонение показывает похожесть на эталонную траекторию; норм. СКО — рассогласование
      ...
      <section className="table-card">
        {compare?.rows ? (
            ...
            <tbody>
              {compare.rows.map((r) => (
                <tr key={r.kind} className={r.kind === 'pn' ? 'ref' : ''}>
                  ...
                  <td data-tip="СКО рассогласования с эталонным ПН, нормированное начальной дальностью. Меньше = ближе к эталону. Это не «процент совпадения».">
          ...
          <p className="empty-state">
            {compareLoading
              ? 'Считаю…'
              : 'Нажмите «Прогнать сравнение»: строки — эталонный ПН, законы наведения, все мозги. Метрики — конечный промах, время наведения, похожесть на эталон, пик перегрузки, доля захвата.'}
          </p>
        )}
      </section>
      ...
      <div>
        ...
        <div className="lab-grid" style={{ marginTop: 6, alignItems: 'start' }}>
          ...
          <section className="lab-subcard">
            ...
            {faults && (
              <>
                ...
                <p className="lab-hint">
                  Промах (м) при отказе 0–80% омматидиев. Пологая кривая — зрение устойчиво; резкий рост — мозг держится на
                  конкретных каналах.
                </p>
              </>
            )}
          ...
          <section className="lab-subcard">
            ...
            <p className="lab-hint">
              Конвейер: учитель → дистиллят → файл → применение → дообучение «по результату». Откл. от ПН у дистиллята
...
export function NeffScreen({ data, onExport }: { data: LabData; onExport: () => void }) {
  ...
  useAutoRedraw(() => {
    ...
    if (c4.current)
      drawChart(
        ...
        'N_экв против скорости сближения V_сбл (кадры по возрастанию V_сбл) — операторская диагностика',
      )
  })
  ...
  return (
    <LabScreen
      ...
      note="Постоянный ПН даёт N_экв ≡ N; если у БИО медиана постоянна, а квартили узки при любом ρ — политика близка к классическому ПН; если N_экв растёт с ρ (при уменьшении t_cpa) — коэффициент адаптивен. На насыщении и при ω≈0 оценка помечается невалидной и в статистику не идёт."
    >
...
/** Экран «Устойчивость к возмущениям»: серия прогонов с нарастающим шумом. */
export function RobScreen({
  ...
  return (
    <LabScreen
      title="Устойчивость к возмущениям"
      ...
      note="Устойчивость к возмущениям считается только на работающем стенде. Серии идут по всем мозгам: схема, полный мозг, коннектом."
    >
      <div className="lab-grid">
        <div className="chart-card"><canvas ref={c1} /></div>
        <div className="chart-card"><canvas ref={c2} /></div>
